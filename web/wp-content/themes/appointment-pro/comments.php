	...
	<?php  
	 $fields=array(
		...
		add_filter('comment_form_default_fields','appointment_fields');
			...
			'logged_in_as' => '<p class="blog-post-info-detail">' . __("Logged in as",'appointment' ).' '.'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'. wp_logout_url( get_permalink() ).'" title="'.__('Log out from this Account','appointment').'">'.' '.__("Log out",'appointment').'?</a>' . '</p>',
