...
if ($total > 1) {
	...
	<div class="blog-pagination">
			...
			<?php 
			for($i=1;$i<=($post_type_data->max_num_pages>1 ? $post_type_data->max_num_pages : 0 );$i++)
			{
				echo '<a class="'.($i == $curpage ? 'active ' : '').'" href="'.get_pagenum_link($i).'">'.$i.'</a>';
			} ?>
		 <?php if($post_type_data->max_num_pages!= $curpage): ?>
		<a href="<?php echo get_pagenum_link(($curpage+1 <= $post_type_data->max_num_pages ? $curpage+1 : $post_type_data->max_num_pages )); ?>"><i class="fa fa-angle-double-right"></i></a>
		<?php endif; ?>
	</div>
<?php } }
}
